...
def parse_args():
    ...
    parser.add_argument("--ports", required=True, help="Plage de ports (ex: 22 ou 20-80) / Port range (e.g. 22 or 20-80)")
...
if __name__ == "__main__":
    # On prépare le chemin vers le fichier log.txt dans le dossier parent / Path to log.txt in parent folder
    parent_log_path = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', 'log.txt'))

    # On configure le carnet de log pour qu’il écrive dans ce fichier / Set up the log to write to that file
    logging.basicConfig(
        level=logging.INFO,
        filename=parent_log_path,
        filemode="w",
